...
$rs = mysqli_query($link, "select r.id,r.dt,r.lid,l.userid,uname,addr,degree,college,l.loanamt,l.proof,n.photo,n.aadhar,n.cproof from reqtobank r,loanapply l,newuser n where r.lid=l.id and l.userid=n.userid and r.astatus='accept' and r.bankid='$_SESSION[bankid]'") or die(mysqli_error($link));
echo "<div class='scrolldiv'><table class='report_tab' style='float:none;margin:auto;min-width:75%;'><thead><tr><th colspan='8'>ACCEPTED LOAN REQUEST LIST";
echo "<tr><th>Req.Dt<th>Student Name<th>Address<th>Degree<th>College<th>Loan Amt<th>Proof</thead><tbody>";
